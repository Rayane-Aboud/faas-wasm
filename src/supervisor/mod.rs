...
use crate::node::node_behavior::Node;


pub struct Supervisor {
  pub nodes: HashMap<Arc<str>, Box<dyn Node>>,
}

impl Supervisor {
  pub fn new()-> Supervisor{
    Supervisor {
      nodes: HashMap::<Arc<str>, Box<dyn Node>>::new(),
    }
  }
  ...
  pub fn get_node_health_summary(&self, id: &str) -> (f32,f32,f32,Option<f32>) {
    if let Some(node) = self.nodes.get(id){
      return node.load_summary();
    }
    (0.0f32,0.0f32,0.0f32,None)
  }
  //death signal
